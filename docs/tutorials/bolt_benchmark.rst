...
* 1,000 diploid individuals and a ``4N = 4,000`` generation burn-in;
* ``V_S = 2N = 2,000``, hence ``W_S = V_S/(2N) = 1``;
...
calibration set. Both blocks retain all 1,000 individuals and their GRGs are
...
when the example is run directly.
...
.. plot:: tutorials/bolt_benchmark.py
   :context: reset
